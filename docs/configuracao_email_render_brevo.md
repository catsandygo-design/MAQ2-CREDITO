...
Para o Foguetinho enviar alertas sem depender dessas portas, use API HTTPS da Brevo.
...
EMAIL_FROM_NAME=Foguetinho SioCred
AI_ASSISTANT_DISPLAY_NAME=Foguetinho
...
3. No painel admin do SioCred, abra `Foguetinho - Alertas por e-mail`.
4. Clique em `Atualizar status`.
5. O canal deve aparecer como `API Brevo`.
6. Clique em `Testar envio`.

Se aparecer `Teste aceito pela API Brevo`, o envio saiu do SioCred. Procure pelo assunto `Foguetinho: teste de alerta por e-mail`.
