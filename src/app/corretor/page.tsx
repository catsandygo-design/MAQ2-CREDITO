    ...
    <article className={`client-row ${tone}`}>
      <div className="client-main">
        ...
        <div className="client-copy">
          ...
          <p>{cliente.empreendimento} | {cliente.corretor}</p>
          <strong>{cliente.acao}</strong>
        </div>
      </div>
      <div className="client-stats">
        <div><span>Etapa</span><strong>{cliente.etapa}</strong></div>
        <div><span>Caixa</span><strong>{cliente.caixa}</strong></div>
        <div><span>Agehab</span><strong>{cliente.agehab}</strong></div>
        <div><span>Docs</span><strong>{cliente.docs}</strong></div>
        <div><span>SLA CCA</span><strong>{cliente.slaCca}</strong></div>
      </div>
      <div className="client-footer">
        <span>Comercial: {cliente.comercial}</span>
        <span>Repasse: {cliente.repasse}</span>
        <span>Sinal: {cliente.sinal}</span>
        <span>Fiador: {cliente.fiador}</span>
  ...
  return (
    <main className="broker-page">
      <style>{`
        ...
        .workspace { display: grid; grid-template-columns: 305px 1fr; gap: 8px; min-height: 0; } .left-rail { min-height: 0; overflow-y: auto; } .right-workstream { display: grid; grid-template-rows: auto minmax(0, 1fr); gap: 8px; min-height: 0; overflow: hidden; }
        .panel { overflow: hidden; } .panel-head { display: flex; justify-content: space-between; gap: 10px; align-items: flex-start; padding: 10px 13px; border-bottom: 1px solid #e5edf5; } .panel-body { padding: 10px 13px; } .count { background: #eef2f7; color: #334155; border-radius: 999px; padding: 5px 9px; font-size: 12px; font-weight: 900; white-space: nowrap; }
