...
function TaskCard({ group }: { group: (typeof taskGroups)[number] }) {
  ...
  return (
    <article className={`task-card ${group.tone}`}>
      <div className="task-head"><span className="task-icon"><Icon size={15} /></span><span className="task-count">{group.items.length}</span></div>
      <h3>{group.title}</h3>
...
function FlowLine({ steps, activeStep }: { steps: string[]; activeStep: number }) {
  return <div className="flow-line" style={{ ['--step-count' as string]: steps.length }}>{steps.map((step, index) => <div className="flow-step" key={step}><span className={index === activeStep ? 'flow-label active' : 'flow-label'}>{step}</span><span className={`flow-dot ${index < activeStep ? 'done' : ''} ${index === activeStep ? 'active' : ''}`} /></div>)}</div>;
}
  ...
  return (
    <article className={`client-flow-card ${tone}`}>
      <div className="client-flow-top">
        <div className="client-identity"><span className={`risk-dot ${tone}`} /><div><div className="client-title-row"><h3>{cliente.cliente}</h3><StatusPill value={cliente.prioridade} tone={tone} /></div><p>{cliente.empreendimento}</p><span>{cliente.corretor}</span></div></div>
      ...
      <div className="client-main-grid">
        <section className="client-process-card"><h4>{cliente.etapa}</h4><p>Repasse em {cliente.repasse}. Docs: {cliente.docs}</p><div className="chip-row"><StatusPill value={`Aging ${cliente.aging}`} /><StatusPill value={`SLA CCA ${cliente.slaCca}`} tone={tone} /></div></section>
      ...
      <div className="client-bottom-grid"><div className="info-tile"><span>Caixa</span><strong>{cliente.caixa}</strong></div><div className="info-tile"><span>Agehab</span><strong>{cliente.agehab}</strong></div><div className="info-tile"><span>Sinal</span><strong>{cliente.sinal}</strong></div><div className="info-tile"><span>Fiador</span><strong>{cliente.fiador}</strong></div><div className="info-tile"><span>SLA</span><strong>{cliente.slaCca}</strong></div><div className="mapped-issues"><h4>Pendencias mapeadas</h4>{cliente.pendencias.map((item) => <p key={item}>{item}</p>)}</div></div>
    </article>
  );
}
  ...
  return (
    <main className="broker-page">
      <style>{`
        ...
        h1, h2, h3, h4, p { margin: 0; } h1 { font-size: 21px; } h2 { font-size: 16px; } h3 { font-size: 13px; } h4 { font-size: 14px; }
        ...
        .btn, .client-actions button { display: inline-flex; align-items: center; justify-content: center; gap: 7px; border: 1px solid #cbd5e1; background: #fff; border-radius: 7px; padding: 8px 10px; font-weight: 800; font-size: 13px; color: #071225; text-decoration: none; cursor: pointer; }
        ...
        .workspace { display: grid; grid-template-columns: 305px 1fr; gap: 8px; min-height: 0; }
        .left-rail { min-height: 0; overflow-y: auto; } .right-workstream { display: grid; grid-template-rows: auto minmax(0, 1fr); gap: 8px; min-height: 0; overflow: hidden; }
        .panel { overflow: hidden; } .panel-head { display: flex; justify-content: space-between; gap: 10px; align-items: flex-start; padding: 10px 13px; border-bottom: 1px solid #e5edf5; } .panel-body { padding: 10px 13px; }
        ...
        .task-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 7px; } .task-card { border: 1px solid #dbe4ef; border-radius: 8px; padding: 8px; background: #fbfdff; } .task-card.critical { border-top: 4px solid #dc2626; } .task-card.high { border-top: 4px solid #f97316; } .task-card.medium { border-top: 4px solid #d97706; } .task-card.info { border-top: 4px solid #0284c7; } .task-card.success { border-top: 4px solid #16a34a; } .task-head { display: flex; justify-content: space-between; margin-bottom: 5px; } .task-icon { width: 25px; height: 25px; border-radius: 7px; display: grid; place-items: center; background: #eef2f7; } .task-count { min-width: 22px; height: 22px; display: grid; place-items: center; border-radius: 999px; background: #e2e8f0; font-weight: 900; } .task-card li { color: #334155; font-size: 11px; line-height: 1.2; } ul { margin: 6px 0 0; padding-left: 15px; display: grid; gap: 3px; }
        .client-list-panel { display: grid; grid-template-rows: auto auto minmax(0, 1fr); min-height: 0; background: linear-gradient(180deg, #ffffff 0%, #f7fbff 100%); } .client-list-actions { justify-content: flex-end; } .client-list-actions .count { background: #fff; border: 1px solid #d8e2ee; padding: 6px 10px; font-size: 13px; }
        ...
        .client-flow-list { min-height: 0; overflow-y: auto; display: grid; align-content: start; gap: 10px; padding: 10px; background: #eef8ff; scrollbar-gutter: stable; } .client-flow-list::-webkit-scrollbar, .left-rail::-webkit-scrollbar { width: 10px; } .client-flow-list::-webkit-scrollbar-thumb, .left-rail::-webkit-scrollbar-thumb { background: #94a3b8; border-radius: 999px; } .client-flow-list::-webkit-scrollbar-track, .left-rail::-webkit-scrollbar-track { background: #e2e8f0; border-radius: 999px; }
        .client-flow-card { background: #fff; border: 1px solid #dbe4ef; border-left-width: 4px; border-radius: 8px; overflow: hidden; box-shadow: 0 12px 28px rgba(15, 23, 42, .06); } .client-flow-card.danger { border-left-color: #dc2626; } .client-flow-card.warn { border-left-color: #d97706; } .client-flow-card.ok { border-left-color: #16a34a; }
        .client-flow-top { display: flex; justify-content: space-between; gap: 12px; padding: 12px 14px; border-bottom: 1px solid #e5edf5; } .client-identity { display: flex; gap: 11px; min-width: 0; } .client-title-row { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; } .client-identity h3 { color: #082f49; font-size: 16px; line-height: 1.2; text-transform: uppercase; } .client-identity p { color: #334155; font-size: 13px; margin-top: 4px; } .client-identity span { color: #475569; font-size: 12px; margin-top: 2px; display: inline-block; } .risk-dot { width: 11px; height: 11px; border-radius: 999px; margin-top: 5px; box-shadow: 0 0 0 5px #eef8ff; flex: 0 0 auto; } .risk-dot.danger { background: #dc2626; } .risk-dot.warn { background: #d97706; } .risk-dot.ok { background: #16a34a; }
        ...
        .client-main-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 10px 14px; background: #f8fbff; } .client-process-card, .client-action-card, .info-tile, .mapped-issues { border: 1px solid #e5edf5; border-radius: 8px; background: #fff; padding: 10px; } .client-action-card { background: #fff7f7; border-color: #fde2e2; } .client-process-card h4, .client-action-card h4 { font-size: 15px; margin-bottom: 5px; } .client-process-card p, .client-action-card p, .mapped-issues p { color: #334155; font-size: 12px; line-height: 1.35; margin-top: 4px; }
        .flow-panel { margin: 10px 14px; border: 1px solid #e5edf5; border-radius: 8px; padding: 9px 12px 12px; background: #f8fbff; } .flow-panel.repasse { background: #f0fdf4; } .flow-heading { display: flex; justify-content: space-between; align-items: center; margin-bottom: 9px; } .flow-heading span { border-radius: 999px; padding: 5px 9px; font-size: 10px; text-transform: uppercase; letter-spacing: .08em; font-weight: 900; color: #075985; background: #e0f2fe; } .flow-panel.repasse .flow-heading span { color: #166534; background: #dcfce7; } .flow-heading strong { font-size: 14px; }
        .flow-line { display: grid; grid-template-columns: repeat(var(--step-count), minmax(72px, 1fr)); position: relative; } .flow-line:before { content: ''; position: absolute; left: 18px; right: 18px; top: 30px; height: 4px; border-radius: 999px; background: #cbd5e1; } .flow-step { display: grid; justify-items: center; gap: 10px; position: relative; min-width: 0; } .flow-label { color: #475569; font-size: 11px; text-align: center; line-height: 1.15; min-height: 26px; } .flow-label.active { color: #020617; font-weight: 900; } .flow-dot { width: 13px; height: 13px; border-radius: 999px; border: 3px solid #cbd5e1; background: #fff; z-index: 1; } .flow-dot.done { border-color: #10b981; background: #10b981; } .flow-dot.active { width: 17px; height: 17px; border-color: #38bdf8; box-shadow: 0 0 0 6px rgba(56, 189, 248, .18); }
        .client-bottom-grid { display: grid; grid-template-columns: repeat(5, minmax(100px, 1fr)) 1.4fr; gap: 8px; padding: 0 14px 14px; } .info-tile span { display: block; color: #475569; font-size: 11px; text-transform: uppercase; letter-spacing: .06em; font-weight: 900; margin-bottom: 6px; } .info-tile strong { display: block; border: 1px solid #93c5fd; background: #dbeafe; color: #075985; border-radius: 999px; text-align: center; padding: 7px; font-size: 12px; } .mapped-issues h4 { color: #082f49; font-size: 14px; }
        @media (max-width: 1180px) { .broker-page { height: auto; overflow: auto; } .broker-shell { height: auto; } .workspace { grid-template-columns: 1fr; } .right-workstream { overflow: visible; } .client-list-panel { min-height: 560px; } .task-grid, .summary-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .client-main-grid, .client-bottom-grid { grid-template-columns: 1fr 1fr; } .mapped-issues { grid-column: 1 / -1; } }
        @media (max-width: 900px) { .flow-line { overflow-x: auto; padding-bottom: 8px; grid-auto-flow: column; grid-auto-columns: 110px; grid-template-columns: none; } .client-flow-top { flex-direction: column; } .client-actions { justify-content: flex-start; } }
        @media (max-width: 760px) { .broker-page { padding: 10px; } .topbar, .actions { align-items: flex-start; flex-direction: column; } .task-grid, .summary-grid, .filters, .client-main-grid, .client-bottom-grid { grid-template-columns: 1fr; } .client-identity h3 { font-size: 15px; } }
      `}</style>
