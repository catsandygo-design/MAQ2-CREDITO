...
import { enviarFormulariosAssinados } from '../../../../../../lib/workflow/engine';
...
export async function POST(request: NextRequest, context: ProcessoRouteContext) {
  return mutateProcesso(request, context, (processo, body) =>
    enviarFormulariosAssinados(
      processo,
      Array.isArray(body.assinados)
        ? (body.assinados as Array<{ formularioId: string; assinadoUrl: string }>)
        : [],
    ),
  );
}
